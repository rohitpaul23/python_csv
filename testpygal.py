...
def build_plot_values(gdpinfo, gdpdata):
    ...
    for row in gdpdata:
        try:
            if gdpinfo["min_year"] <= int(row) and int(row) <= gdpinfo["max_year"]:
                if gdpdata[row] != '':
                    gdplist.append((int(row), float(gdpdata[row])))
        except ValueError:
            continue
    return gdplist
...
def build_plot_dict(gdpinfo, country_list):
    ...
    """
    plot_dict = {}
    nest_dict_list = read_csv_as_nested_dict(gdpinfo["gdpfile"],
                                             gdpinfo["country_name"],
                                             gdpinfo["separator"],
                                             gdpinfo["quote"])
    for row in country_list:
        gdpdata = {}
        if row in nest_dict_list.keys():
            for year in range(gdpinfo["min_year"], gdpinfo["max_year"]+1):
                gdpdata[year] = nest_dict_list[row][str(year)]
            gdplist = build_plot_values(gdpinfo, gdpdata)
        else:
            gdplist = []
        plot_dict[row] = gdplist
    return plot_dict
...
def render_xy_plot(gdpinfo, country_list, plot_file):
    ...
    for row in plot_dict:
        xyplot.add(row, plot_dict[row])
...
#test_render_xy_plot()
# print(build_plot_dict({'gdpfile': 'gdpfile.csv', 'separator': ',', 'quote': '"',
#                        'min_year': 2000, 'max_year': 2005, 'country_name': 'Country Name',
#                        'country_code': 'Code'}, ['Country2']))